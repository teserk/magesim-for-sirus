...
double Player::baseCastTime(std::shared_ptr<spell::Spell> spell) const {
  ...
  if (spell->id == spell::PYROBLAST && (hasBuff(buff::HOT_STREAK) || hasBuff(buff::FIRE_POWER)))
    t = 0;
...
double Player::critChance(std::shared_ptr<spell::Spell> spell) const {
  ...
  // TODO: Разделить зандаларов на ауры, пока предполагается что зандалары ходят с критом
  if (race == RACE_ZANDALARI) {
    crit += 2;
  }
...
double Player::buffDmgMultiplier(std::shared_ptr<spell::Spell> spell, const State &state) const {
  ...
  if (hasBuff(buff::LIGHT_FORGED))
    multi *= 1.2;
  if (hasBuff(buff::BLOOD_ELF_RACE))
    multi *= 1.1;
  if (race == RACE_LIGHTFORGED)
    multi *= 1.02;
  if (config.udc_2set)
    multi *= 1.01;
  if (config.udc_3set)
  ...
  if (spell->id == spell::FROSTFIRE_BOLT && glyphs.frostfire)
    additive += 0.02;
  if (config.t6_4set &&
      (spell->id == spell::FIREBALL || spell->id == spell::FROSTBOLT || spell->id == spell::ARCANE_MISSILES))
    additive += 0.05;
...
double Player::getSpellPower(School school) const {
  ...
  if (race == RACE_ZANDALARI || race == RACE_LIGHTFORGED)
    sp *= 1.01;
...
std::vector<action::Action> Player::onCastStartProc(const State &state, std::shared_ptr<spell::Spell> spell,
  ...
  if (config.t10_2set) {
    ...
    else if ((spell->id == spell::FROSTFIRE_BOLT || spell->id == spell::FIREBALL) && hasBuff(buff::BRAIN_FREEZE) &&
             spell->actual_cast_time == 0)
      actions.push_back(buffAction<buff::PushingTheLimit>());
  }
...
std::vector<action::Action> Player::onCastSuccessProc(const State &state, std::shared_ptr<spell::Spell> spell,
  ...
  if (hasBuff(buff::BRAIN_FREEZE) && spell->actual_cast_time == 0 &&
      (spell->id == spell::FROSTFIRE_BOLT || spell->id == spell::FIREBALL)) {
  ...
  if (spell->can_proc) {
    ...
    if (hasTrinket(TRINKET_FLARE_HEAVENS) && !hasCooldown(cooldown::FLARE_HEAVENS) && is_harmful &&
        random<int>(0, 9) == 0) {
    ...
    if ((config.black_magic || black_magic) && !hasCooldown(cooldown::BLACK_MAGIC) && spell->channeling &&
        is_harmful && spell->id != spell::BLIZZARD && random<int>(0, 99) < 35) {
    ...
    if (hasTrinkets(TRINKET_FETISH_VOLATILE_POWER_HC, TRINKET_TALISMAN_VOLATILE_POWER_HC) &&
        hasBuff(buff::VOLATILE_POWER_HC) && is_harmful)
      actions.push_back(buffAction<buff::VolatilityHc>());
    if (hasTrinkets(TRINKET_FETISH_VOLATILE_POWER_NM, TRINKET_TALISMAN_VOLATILE_POWER_NM) &&
        hasBuff(buff::VOLATILE_POWER_NM) && is_harmful)
    ...
    // Unconfirmed - on spell cast ?
    if (hasTrinket(TRINKET_PURIFIED_LUNAR_DUST) && !hasCooldown(cooldown::PURIFIED_LUNAR_DUST) &&
        random<int>(0, 9) == 0) {
  ...
  if (config.rot_black_magic && spell->actual_cast_time == 0 && is_harmful) {
    if (black_magic && canReactTo(cooldown::BLACK_MAGIC, state.t) ||
        !black_magic && !hasCooldown(cooldown::BLACK_MAGIC)) {
      swapWeapons();
    }
  }
...
std::vector<action::Action> Player::onProcSuccess(const State &state, std::shared_ptr<spell::Spell> spell,
                                                  std::shared_ptr<target::Target> target) {
...
std::vector<action::Action> Player::onSpellImpactProc(const State &state, const spell::SpellInstance &instance,
  ...
  if (instance.spell->id == spell::LIVING_BOMB && instance.tick == instance.spell->ticks) {
    ...
    if (hasTrinkets(TRINKET_FETISH_VOLATILE_POWER_HC, TRINKET_TALISMAN_VOLATILE_POWER_HC) &&
        hasBuff(buff::VOLATILE_POWER_HC))
      actions.push_back(buffAction<buff::VolatilityHc>());
    if (hasTrinkets(TRINKET_FETISH_VOLATILE_POWER_NM, TRINKET_TALISMAN_VOLATILE_POWER_NM) &&
        hasBuff(buff::VOLATILE_POWER_NM))
      actions.push_back(buffAction<buff::VolatilityNm>());
  ...
  if (instance.result != spell::MISS) {
    ...
    if (instance.spell->id == spell::IGNITE && talents.empowered_fire) {
      ...
      if (chance == 100 || random<int>(0, 99) < chance)
        actions.push_back(manaAction(maxMana() * 0.01, "Ignite"));// Increased mana regen as in Sirus
    }
    // Deleted glyph check on Sirus
    ...
    if (instance.spell->dot) {
      if (hasTrinket(TRINKET_EXTRACT_NECROMANTIC_POWER) && !hasCooldown(cooldown::EXTRACT_NECROMANTIC_POWER) &&
          random<int>(0, 9) == 0)
        actions.push_back(
          spellCooldownAction<spell::ExtractNecromanticPower, cooldown::ExtractNecromanticPower>(target));
      ...
      if (hasBuff(buff::FIRE_POWER) &&
          instance.spell->id == spell::ID::PYROBLAST) {
        fire_power++;
        if (fire_power == 3) {
          actions.push_back(buffExpireAction<buff::FirePower>());
          actions.push_back(cooldownAction<cooldown::FirePower>());
        }
      }

      if (hasBuff(buff::COMBUSTION) &&
          (instance.spell->school == SCHOOL_FIRE || instance.spell->school == SCHOOL_FROSTFIRE)) {
      ...
      if (config.lightweave_embroidery && !instance.spell->channeling && !hasCooldown(cooldown::LIGHTWEAVE) &&
          random<int>(0, 99) < 35) {
  ...
  if (instance.result == spell::CRIT) {
    ...
    if (!instance.spell->dot) {
      ...
      if (hasTrinkets(TRINKET_REIGN_DEAD_HC, TRINKET_REIGN_UNLIVING_HC) &&
          !hasCooldown(cooldown::REIGN_UNLIVING_HC) && instance.spell->id != spell::PILLAR_OF_FLAME_HC) {
        if (buffStacks(buff::REIGN_UNLIVING_HC) == 2) {
          ...
          actions.push_back(cooldownAction<cooldown::ReignUnlivingHc>());
        } else {
          actions.push_back(buffCooldownAction<buff::ReignUnlivingHc, cooldown::ReignUnlivingHc>());
        }
      }
      if (hasTrinkets(TRINKET_REIGN_DEAD_NM, TRINKET_REIGN_UNLIVING_NM) &&
          !hasCooldown(cooldown::REIGN_UNLIVING_NM) && instance.spell->id != spell::PILLAR_OF_FLAME_NM) {
        if (buffStacks(buff::REIGN_UNLIVING_NM) == 2) {
          ...
          actions.push_back(cooldownAction<cooldown::ReignUnlivingNm>());
        } else {
...
std::vector<action::Action> Player::onSpellTickProc(const State &state, std::shared_ptr<spell::Spell> spell,
                                                    std::shared_ptr<target::Target> target, int tick) {
...
Player::onCastOrTick(const State &state, std::shared_ptr<spell::Spell> spell, std::shared_ptr<target::Target> target,
                     int tick) {
  std::vector<action::Action> actions;
  ...
  if (spell->max_dmg > 0) {
    ...
    if (hasTrinket(TRINKET_CHARRED_TWILIGHT_SCALE_HC) && !hasCooldown(cooldown::CHARRED_TWILIGHT_SCALE_HC) &&
        random<int>(0, 9) == 0) {
      actions.push_back(buffCooldownAction<buff::CharredTwilightScaleHc, cooldown::CharredTwilightScaleHc>());
    }
    if (hasTrinket(TRINKET_CHARRED_TWILIGHT_SCALE_NM) && !hasCooldown(cooldown::CHARRED_TWILIGHT_SCALE_NM) &&
        random<int>(0, 9) == 0) {
    ...
    // Unconfirmed - on harmful ?
    if (hasTrinket(TRINKET_PENDULUM_TELLURIC_CURRENTS) && !hasCooldown(cooldown::PENDULUM_TELLURIC_CURRENTS) &&
        random<int>(0, 19) < 3) {
      if (target == NULL)
        target = state.targets[random<int>(0, state.targets.size() - 1)];
      actions.push_back(
        spellCooldownAction<spell::PendulumTelluricCurrents, cooldown::PendulumTelluricCurrents>(target));
    }

    // Unconfirmed - on harmful ?
    if (hasTrinket(TRINKET_ELEMENTAL_FOCUS_STONE) && !hasCooldown(cooldown::ALACRITY_ELEMENTS) &&
        random<int>(0, 9) == 0) {
      actions.push_back(buffCooldownAction<buff::AlacrityElements, cooldown::AlacrityElements>());
    }

    // Unconfirmed - on harmful ?
    if (hasTrinket(TRINKET_DISLODGED_OBJECT_HC) && !hasCooldown(cooldown::DISLODGED_OBJECT_HC) &&
        random<int>(0, 9) == 0) {
      actions.push_back(buffCooldownAction<buff::DislodgedObjectHc, cooldown::DislodgedObjectHc>());
    }
    if (hasTrinket(TRINKET_DISLODGED_OBJECT_NM) && !hasCooldown(cooldown::DISLODGED_OBJECT_NM) &&
        random<int>(0, 9) == 0) {
...
bool Player::useTimingIfPossible(const std::string &name, const State &state, bool expl) {
  ...
  if (timing == nullptr && !expl)
    return true;

  if (timing != nullptr && isTimingReady(timing, state)) {
    useTiming(timing);
    return true;
  }
...
// Бро???? Че это за хуйня? какой додик это писал?
// лан не так уж и хуево
action::Action Player::useCooldown(const State &state) {
  if (state.isMoving()) {
    if (talents.presence_of_mind && !hasCooldown(cooldown::PRESENCE_OF_MIND) && !hasBuff(buff::ARCANE_POWER) &&
  ...
  if (config.t10_4set && !hasCooldown(cooldown::MIRROR_IMAGE) && useTimingIfPossible("mirror_image", state) &&
      state.duration - state.t >= 10.0) {
    return spellCooldownAction<spell::MirrorImage, cooldown::MirrorImage>();
  } else if (talents.arcane_power && !hasCooldown(cooldown::ARCANE_POWER) && !hasBuff(buff::PRESENCE_OF_MIND) &&
             useTimingIfPossible("arcane_power", state)) {
  ...
  } else if (talents.cold_snap && !hasCooldown(cooldown::COLD_SNAP) &&
             useTimingIfPossible("cold_snap", state, true)) {
  ...
  } else if (race == RACE_BLOOD_ELF && !hasCooldown(cooldown::ARCANE_TORRENT) && manaPercent() <= 94.0 &&
             useTimingIfPossible("arcane_torrent", state)) {
    return spellAction<spell::ArcaneTorrent>();// nice
  } else if (config.hyperspeed_accelerators && !hasCooldown(cooldown::HYPERSPEED_ACCELERATION) &&
             useTimingIfPossible("hyperspeed_accelerators", state)) {
    return buffCooldownAction<buff::HyperspeedAcceleration, cooldown::HyperspeedAcceleration>(true);
  } else if (config.potion != POTION_NONE && config.potion != POTION_MANA && !hasCooldown(cooldown::POTION) &&
             useTimingIfPossible("potion", state)) {
    action::Action action{action::TYPE_POTION};
    action.potion = config.potion;
    action.primary_action = true;
    return action;
  } else if (config.potion == POTION_MANA && !hasCooldown(cooldown::POTION) &&
             useTimingIfPossible("potion", state, true)) {
    action::Action action{action::TYPE_POTION};
    action.potion = config.potion;
    action.primary_action = true;
    return action;
  } else if (config.conjured == CONJURED_DARK_RUNE && !hasCooldown(cooldown::CONJURED) && (useTimingIfPossible("conjured", state, true) || (talents.incanters_absorption && config.pre_incanters_absorption && config.pre_mana_incanters_absorption && hasBuff(buff::MANA_SHIELD) && hasBuff(buff::ARCANE_POWER) && getNextTiming("conjured") == NULL) || ((!talents.incanters_absorption || !config.pre_incanters_absorption || !config.pre_mana_incanters_absorption) && maxMana() - mana > 1500 && useTimingIfPossible("conjured", state)))) {
  ...
  } else if (!hasCooldown(cooldown::SAPPER_CHARGE) && (useTimingIfPossible("sapper_charge", state, true) ||
                                                       (talents.incanters_absorption && config.pre_incanters_absorption &&
                                                        (hasBuff(buff::FIRE_WARD) || hasBuff(buff::MANA_SHIELD)) && hasBuff(buff::ARCANE_POWER) &&
                                                        getNextTiming("sapper_charge") == NULL))) {
    return spellCooldownAction<spell::SapperCharge, cooldown::SapperCharge>();
  } else if (isUseTrinket(config.trinket1) && !hasCooldown(cooldown::TRINKET1) &&
             !isTrinketOnSharedCD(config.trinket1) && useTimingIfPossible("trinket1", state)) {
    ...
    action.primary_action = true;
    return action;
  } else if (isUseTrinket(config.trinket2) && !hasCooldown(cooldown::TRINKET2) &&
             !isTrinketOnSharedCD(config.trinket2) && useTimingIfPossible("trinket2", state)) {
    action::Action action{action::TYPE_TRINKET};
  ...
  } else if (talents.water_elemental && !hasCooldown(cooldown::WATER_ELEMENTAL) &&
             !state.hasUnit(unit::WATER_ELEMENTAL) && useTimingIfPossible("water_elemental", state)) {
...
std::shared_ptr<spell::Spell> Player::preCastSpell() {
  ...
  if (config.rotation == ROTATION_ST_FROSTFIRE)
    return std::make_shared<spell::FrostfireBolt>();
...
double Player::preCombatDuration(const State &state) {
  ...
  if (talents.incanters_absorption && config.pre_incanters_absorption) {
    t += _gcd;
  ...
  if (shouldPreCast()) {
    ...
    if (config.pre_potion == POTION_SPEED) {
      buff_stats.haste_rating += 500;
      t += castTime(spell);
      buff_stats.haste_rating -= 500;
    } else {
      t += castTime(spell);
...
action::Action Player::preCombat(const State &state) {
  if (talents.incanters_absorption && config.pre_incanters_absorption && config.pre_mana_incanters_absorption &&
      !hasBuff(buff::MANA_SHIELD)) {
  ...
  if (shouldPreCast()) {
    if (config.pre_potion && !hasCooldown(cooldown::POTION)) {
      action::Action action{action::TYPE_POTION};
      action.potion = config.pre_potion;
      return action;
    } else if (!has_pre_cast) {
      has_pre_cast = true;
...
action::Action Player::nextAction(const State &state) {
  ...
  // Fire rotations
  if (config.rotation == ROTATION_ST_FIRE || config.rotation == ROTATION_ST_FROSTFIRE ||
      config.rotation == ROTATION_ST_FIRE_SC) {

    bool hot_streak = canReactTo(buff::HOT_STREAK, state.t) && talents.pyroblast;
    bool fire_power = canReactTo(buff::FIRE_POWER, state.t) && talents.combustion;
    ...
    else if (config.rotation == ROTATION_ST_FROSTFIRE)
      main_spell = std::make_shared<spell::FrostfireBolt>();
    else
    ...
    if (hasBuff(buff::GHOST_FINGERS)) {
      if (talents.deep_freeze && !hasCooldown(cooldown::DEEP_FREEZE)) {
        ...
        return spellAction<spell::IceLance>(target);
               ...
               15.0 - (state.t - t_brain_freeze) <= config.rot_brain_freeze_hold) {
      if (config.rot_brain_freeze_fireball)
        return spellAction<spell::Fireball>(target);
      return spellAction<spell::FrostfireBolt>(target);
  ...
  // Flamestrike
    ...
    if (t_flamestrike + 8.0 - ct > state.t && t_flamestrike_dr + 8.0 - ct <= state.t &&
        state.t + 8.0 < state.duration)
      return spellAction<spell::FlamestrikeDR>();
    return spellAction(fs);
  }
