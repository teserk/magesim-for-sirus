...
namespace unit {
  ...
  double Unit::castHaste() const {
    ...
    if (hasBuff(buff::ICY_VEINS))
      haste *= 1.2;
    if (hasBuff(buff::ZANDALARI_HASTE))
      haste *= 1.1;
  ...
  std::vector<action::Action> Unit::onCastStartProc(const State &state, std::shared_ptr<spell::Spell> spell,
  ...
  Unit::onSpellTickProc(const State &state, std::shared_ptr<spell::Spell> spell, std::shared_ptr<target::Target> target,
                        int tick) {
    return {};
  }

  std::vector<action::Action>
  Unit::onProcSuccess(const State &state, std::shared_ptr<spell::Spell> spell, std::shared_ptr<target::Target> target) {
    return {};
  }
