    ...
    if (logging_spells) {
      for (auto &spell: state.spells) {
        if (spells.find(spell.first) == spells.end()) {
          spells[spell.first].name = spell.second.name;
          spells[spell.first].source = spell.second.source;
          spells[spell.first].min_dmg = spell.second.min_dmg;
          spells[spell.first].max_dmg = spell.second.max_dmg;
        }
        spells[spell.first].casts += (spell.second.casts - spells[spell.first].casts) / (i + 1.0);
        spells[spell.first].misses += (spell.second.misses - spells[spell.first].misses) / (i + 1.0);
        spells[spell.first].hits += (spell.second.hits - spells[spell.first].hits) / (i + 1.0);
        spells[spell.first].crits += (spell.second.crits - spells[spell.first].crits) / (i + 1.0);
        spells[spell.first].dmg += (spell.second.dmg - spells[spell.first].dmg) / (i + 1.0);
        if (spell.second.max_dmg > spells[spell.first].max_dmg)
          spells[spell.first].max_dmg = spell.second.max_dmg;
        if (spell.second.min_dmg < spells[spell.first].min_dmg)
          spells[spell.first].min_dmg = spell.second.min_dmg;
      }
    }
  ...
  if (spell->id == spell::IGNITE) {
    ...
    if (spell->tick_refresh) {
      ...
      for (auto &event: queue) {
        if (event.type == EVENT_SPELL_IMPACT && event.instance.spell->id == spell->id && event.unit == unit && event.target == target) {
          queued_ticks++;
          last_tick = event.t;
        }
      }
