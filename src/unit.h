...
namespace unit {
  ...
  public:
    ...
    virtual std::vector<action::Action>
    onCastStartProc(const State &state, std::shared_ptr<spell::Spell> spell, std::shared_ptr<target::Target> target);

    virtual std::vector<action::Action>
    onCastSuccessProc(const State &state, std::shared_ptr<spell::Spell> spell, std::shared_ptr<target::Target> target);

    virtual std::vector<action::Action>
    onSpellImpactProc(const State &state, const spell::SpellInstance &instance, std::shared_ptr<target::Target> target);

    virtual std::vector<action::Action>
    onSpellTickProc(const State &state, std::shared_ptr<spell::Spell> spell, std::shared_ptr<target::Target> target,
                    int tick);

    virtual std::vector<action::Action>
    onProcSuccess(const State &state, std::shared_ptr<spell::Spell> spell, std::shared_ptr<target::Target> target);
    ...
    template<typename T, typename... Args>
    action::Action
    debuffAction(std::shared_ptr<target::Target> target, bool primary_action = false, Args... args) const;
